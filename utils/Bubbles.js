...
function Bubbles(container, self, options) {
  ...
  this.charting = function(convo, here, intent) {
    _convo = Object.assign(_convo, convo)
    match_key = intent;
    this.paint(_convo[here],intent)
    here ? (standingAnswer = here) : false
  }
  ...
  this.reply = function(turn, category, dialogState) {
    ...
    if (turn.reply !== undefined) {
      ...
      for (var i = turn_reply_length; i < turn.reply.length; i++) {
          ;(function(el, count) {
          questionsHTML +=
              '<span class="bubble-button overTop_answer" data-more='+i+' style="display:none; animation-delay: 30ms;" onClick="' +
              self +
              ".answer('" +
              el.answer +
              "');this.classList.add('bubble-pick')\">" +
    ...
    orderBubbles(turn.says, dialogState, function() {
      bubbleTyping.classList.remove("imagine")
      questionsHTML !== ""
        ? addBubble(questionsHTML, function() {}, "reply", dialogState)
        : bubbleTyping.classList.add("imagine")
    })
  }

  this.paint = function(turn, intent) {
    turn = typeof turn !== "undefined" ? turn : _convo.ice
    questionsHTML = ""
    if (turn.reply !== undefined) {
      turn.reply.reverse()
    ...
    addBubble(turn.says, function() {}, '' ,intent)
  }
  // navigate "answers"
  this.answer = function(key) {
    ...
    _convo[key] !== undefined
      ? (this.reply(_convo[key],dialogState_data), (standingAnswer = key))
      : func(key)
  }
  ...
  var orderBubbles = function(q, dialogState, callback) {
    var start = function() {
      setTimeout(function() {
        callback()
      }, animationTime)
    ...
    for (var nextCallback = position + q.length - 1;nextCallback >= position;nextCallback--) {
      (function(callback, index) {
        start = function() {
          addBubble(q[index], callback, '', dialogState)
        }
      })(start, nextCallback)
    }
    start()
  ...
  var addBubble = function(say, posted, reply, dialogState) {
    ...
    if (reply !== "") {
      ...
      $('.bubble-button').on('click',function(){
        reply_count += 1;
        $(this).siblings('.bubble-button').addClass('bubble-picked');
...
function show_delve_more(){
  result = 1;
  for(i=delve_begin;i<delve_begin+10;i++){
    $('.overTop_answer[data-more='+i+']').show();
  }
  console.log(more_delvel_data)
  console.log(delve_begin)
  if(more_delvel_data['reply'].length<delve_begin+10){
    result = 0;
  }
  else{
    delve_begin += 10;
